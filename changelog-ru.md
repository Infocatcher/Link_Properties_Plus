...
`*` Окно свойств: для формирования заголовка используется прямая ссылка, если доступна (которая в большинстве случаев лучше читается).<br>
`x` Исправлено переключение на уже открытое окно свойств с декодированным URI и/или referer (настройка <em>extensions.linkPropertiesPlus.decodeURIs</em> = true и некоторые не латинские символы в ссылке, например <a href="https://ru.wikipedia.org/wiki/Заглавная_страница">https://ru.wikipedia.org/wiki/Заглавная_страница</a>).<br>
`*` Улучшено поле для заголовков HTTP: клик средней кнопкой мыши для сворачивания/разворачивания всех секций (в дополнение к клику левой кнопкой мыши с любым модификатором).<br>
`+` Добавлена поддержка расширения <a href="https://addons.palemoon.org/addon/properties-dialog/">Properties dialog</a>.<br>
`*` Улучшено определение расширений для восстановления диалога свойств ссылки старого формата для отображения соответствующих настроек.<br>
